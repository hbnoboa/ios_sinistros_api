          ...
          >
            <Form.Group className="mb-3">
              ...
              <Form.Control
                type="datetime-local"
                name="authDate"
                value={reg.authDate ? reg.authDate.slice(0, 16) : ""}
              ...
              <Form.Control
                ...
                value={
                  reg.estimatedArrivalDate
                    ? reg.estimatedArrivalDate.slice(0, 16)
                    : ""
                }
                onChange={(e) => handleRegulatorChange(idx, e)}
              />
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control
                type="datetime-local"
                name="arrivalDate"
                value={reg.arrivalDate ? reg.arrivalDate.slice(0, 16) : ""}
                onChange={(e) => handleRegulatorChange(idx, e)}
              />
            </Form.Group>
            <Form.Group className="mb-3">
              ...
              <Form.Control
                type="datetime-local"
                name="finalDate"
                value={reg.finalDate ? reg.finalDate.slice(0, 16) : ""}
                onChange={(e) => handleRegulatorChange(idx, e)}
              />
            </Form.Group>
            <Form.Group className="mb-3">
            ...
            <Form.Group className="mb-3">
              ...
              {/* Mostrar arquivos já existentes (strings) */}
              {Array.isArray(reg.Attachments_filename) &&
                reg.Attachments_filename.length > 0 &&
                reg.Attachments_filename.some((f) => typeof f === "string") && (
                  <div style={{ fontSize: 12, marginBottom: 4 }}>
                    <b>Arquivos já enviados:</b>
                    <ul>
                      {reg.Attachments_filename.filter(
                        (f) => typeof f === "string"
                      ).map((f, i) => (
                        <li key={i}>
                          <a
              ...
              />
              {/* Mostrar arquivos selecionados para upload */}
              {Array.isArray(reg.Attachments_filename) &&
                reg.Attachments_filename.length > 0 &&
                reg.Attachments_filename.some((f) => typeof f === "object") && (
...
export default RegulatorsEditTabs;
